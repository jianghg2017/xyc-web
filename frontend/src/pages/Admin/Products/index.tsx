...
const CATEGORY_OPTIONS = ['全部', '软件产品', '硬件产品', '解决方案']
...
const ProductsList: React.FC = () => {
  ...
  // --- Fetch ---
  const fetchProducts = useCallback(async (currentPage: number, currentFilters: typeof filters) => {
    setLoading(true)
    try {
  ...
  const handleBatchConfirm = async () => {
    ...
    try {
      await adminApi.products.batchOperation({ action: batchConfirm, ids: selectedIds })
  ...
  return (
    <div className="space-y-4">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="w-16 px-4 py-3 text-left font-medium text-gray-600">封面</th>
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-600 whitespace-nowrap">浏览量</th>
                ...
                {products.map(item => {
                  ...
                  const coverSrc = item.cover_image
                    ? item.cover_image.startsWith('/')
                      ? `${import.meta.env.VITE_API_URL?.replace('/api', '') || 'http://localhost:3000'}${item.cover_image}`
                      : item.cover_image
                    : null
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div
                          className={`w-10 h-10 rounded border border-gray-200 bg-gray-100 items-center justify-center ${coverSrc ? 'hidden' : 'flex'}`}
                          aria-hidden="true"
                        >
                      ...
                      {/* Name */}
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-800 line-clamp-1 max-w-xs" title={item.name}>
                          {item.name}
                        </div>
                        {item.description && (
      ...
      <ConfirmModal
        ...
        title="批量删除"
        message={`确定要删除选中的 ${selectedIds.length} 个产品吗？此操作不可撤销。`}
      ...
      <ConfirmModal
        ...
        title="批量发布"
        message={`确定要发布选中的 ${selectedIds.length} 个产品吗？`}
        onConfirm={handleBatchConfirm}
        onCancel={handleBatchCancel}
        loading={batchLoading}
      />
